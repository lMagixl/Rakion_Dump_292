...
void __cdecl FUN_36202120(int param_1,int param_2,uint param_3,uint param_4)
      ...
      *(byte *)(param_2 + 1) = bVar5;
      if (((bVar1 & 7) != 0) && (bVar5 != 0xf8)) {
        *(byte *)(param_2 + 1) =
             (char)*(undefined4 *)
                    (&DAT_362afbd4 +
                    ((param_3 & 3) + ((param_4 & 3) + (bVar1 & 7) * 4 + iVar2 * -0x1c) * 4) * 4) *
             '\b' + bVar5;
      }
    }
    else if (uVar4 == 3) {
      *(byte *)(param_2 + 3) = *(byte *)(param_1 + 3) & 0x80;
    }
    else {
